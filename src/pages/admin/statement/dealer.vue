...
<script>
   ...
   export default {
      ...
      methods:{
         loadData(){
            this.loading = true
            httpGet(`report/dealer`).then(res=>{
               for (let [key, val] of Object.entries(res.data)) {
                  key == 0 ? this.thead = val : this.dealerList[key] = val;
               }
               this.Loading = false
            })
         },
         searchBtn(){
            this.loading = true
            httpGet(`report/dealer?begin_time=${this.begin_time/1000}&end_time=${this.end_time/1000}`).then(res=>{
               for (let [key, val] of Object.entries(res.data)) {
                  key == 0 ? this.thead = val : this.dealerList[key] = val;
               }
               this.Loading = false
         ...
         started(e){
            ...
            this.$router.push({name:'admin-statement-detail', query:{id,time,field:'pid'}})
         }
      }
   }
</script>
...
<style>
.dealer .priceFrom .el-button{margin-left:4px;}
.dealer .d2-container-full__header .el-form-item{margin:0;}
.dealer .d2-container-full__header .el-input{width:180px;}
</style>