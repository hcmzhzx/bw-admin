...
<script>
   ...
   // 运营
   export default {
      name: 'admin-statement-operation',
      data(){
         return{
            thead:[],
            operationList:{},
      ...
      methods:{
         loadData(){
            this.loading = true
            httpGet(`report/operate`).then(res=>{
               for (let [key, val] of Object.entries(res.data)) {
                  key == 0 ? this.thead = val : this.operationList[key] = val;
               }
               this.Loading = false
            })
         },
         searchBtn(){
            this.loading = true
            httpGet(`report/operate?begin_time=${this.begin_time/1000}&end_time=${this.end_time/1000}`).then(res=>{
               for (let [key, val] of Object.entries(res.data)) {
                  key == 0 ? this.thead = val : this.operationList[key] = val;
               }
               this.Loading = false
...
<style>
.operation .priceFrom .el-button{margin-left:4px;}
.operation .d2-container-full__header .el-form-item{margin:0;}
.operation .d2-container-full__header .el-input{width:180px;}
</style>